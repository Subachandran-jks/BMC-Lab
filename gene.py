...
folder_name = "USA Strains"  # folder name or path of folder
...
if ref == "":
    ...
    with open(os.path.join(folder_name, ref), "r") as file1:  # opening file in read mode
        for line in file1.readlines():  # reading file by line by line
    ...
    with open(os.path.join(folder_name, "/freq_of_" + comparison + "_strains.csv"), "w+") as file3:  # opening output file in w mode
        ...
        for i in freq.keys():  # looping freq dictionary keys (PATRIC ID)
            ...
            for j in freq[i]:  # looping values of freq dictionary(list of count and antibiotics name)
                ...
                # Reason: In csv if comma is detected it seperates them into 2 columns. to avoid that we group them
                # with " example: text1, text2 -> text1 is one column , text2 is another column "text1,
                # text2" -> "text1, text2" are same column
                file3.write(string2 + ",")  # seperate column by comma
            file3.write("\n")  # seperating row by \n
